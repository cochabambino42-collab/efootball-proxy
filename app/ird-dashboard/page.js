            ...
            <div className="flex items-center gap-4 mt-2">
              <div>
                ...
                <div className="w-48 h-2 bg-gray-700 rounded-full overflow-hidden mt-1">
                  <div className="h-full bg-gradient-to-r from-green-500 to-cyan-500" style={{ width: `${dailyUsage}%` }}></div>
                </div>
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500" style={{ width: `${memoryUsage}%` }}></div>
                </div>
              </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
            ...
            <div className="mt-6 p-3 bg-gray-900/70 rounded-lg border border-cyan-500/20">
              ...
              <p className="text-xs text-gray-400 mt-1">Determinismo quirúrgico. Cada request es verificable, cada límite es explotado tácticamente.</p>
            </div>
          </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
            ...
            <div className="space-y-2">
              {exampleUrls.map((exampleUrl, index) => (
                <button key={index} onClick={() => handleExampleClick(exampleUrl)} className="w-full text-left p-3 rounded-lg hover:bg-gray-700/50 transition-colors border border-gray-600/50 hover:border-green-500/50">
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input type="url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://efootballhub.net/..." className="flex-1 px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500" />
                <button onClick={handleScrape} disabled={loading} className="px-6 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                  {loading ? 'SCRAPEANDO...' : 'EJECUTAR SCRAPING'}
                </button>
              </div>
              <p className="text-xs text-gray-500 mt-2">Solo dominios permitidos: <span className="text-cyan-400">efootballhub.net</span></p>
            </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-4">
                    <span className={`px-3 py-1 rounded-full text-sm ${result.cache === 'HIT' ? 'bg-green-900/50 text-green-400' : 'bg-yellow-900/50 text-yellow-400'}`}>CACHE: {result.cache}</span>
                    <button onClick={clearResults} className="px-3 py-1 text-sm bg-gray-700 hover:bg-gray-600 rounded-lg">Limpiar</button>
                  </div>
                </div>
                ...
                {result.data?.stats && (
                  <div className="p-4 bg-gray-900/70 rounded-lg">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="text-center"><p className="text-2xl font-bold text-blue-400">{result.data.stats.links || 0}</p><p className="text-xs text-gray-400">Enlaces</p></div>
                      <div className="text-center"><p className="text-2xl font-bold text-purple-400">{result.data.stats.images || 0}</p><p className="text-xs text-gray-400">Imágenes</p></div>
                      <div className="text-center"><p className="text-2xl font-bold text-green-400">{result.data.stats.size || 0}</p><p className="text-xs text-gray-400">Bytes</p></div>
                    </div>
                  </div>
                )}
                ...
                <div className="p-4 bg-gray-900/70 rounded-lg">
                  ...
                  <pre className="text-xs bg-black/50 p-3 rounded-lg overflow-x-auto text-gray-300 font-mono">{result.data?.htmlPreview || 'No hay preview disponible'}</pre>
                </div>

                <div className="p-4 bg-gray-900/70 rounded-lg">
                  <p className="text-gray-400 mb-2">🕒 Metadatos del Request</p>
                  <div className="flex flex-wrap gap-4 text-sm">
                    <div><span className="text-gray-500">Timestamp:</span><span className="ml-2 text-cyan-300">{result.timestamp || 'N/A'}</span></div>
                    <div><span className="text-gray-500">Éxito:</span><span className={`ml-2 ${result.success ? 'text-green-400' : 'text-red-400'}`}>{result.success ? 'SÍ' : 'NO'}</span></div>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="text-center py-12">
                ...
                <div className="mt-6 flex justify-center">
                  <div className="text-left space-y-2 text-sm text-gray-500">
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500"></div><span>Cache LRU activo (30min TTL)</span></div>
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-blue-500"></div><span>Rate limiting: {IRD_CONFIG.RATE_LIMIT_PER_MINUTE}/min</span></div>
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-cyan-500"></div><span>Timeout configurado: 8 segundos</span></div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                ...
                <code className="block mt-1 px-3 py-2 bg-black/50 rounded-lg text-cyan-300 text-sm">POST https://efootball-proxy-jkja.vercel.app/api/ird/proxy</code>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">Operación Cimientos • I.R.D. Protocol • v1.0.0</p>
                <p className="text-xs text-gray-600 mt-1">Arquitectura 100% Free Tier • Costo garantizado $0.00</p>
              </div>
            </div>
          </div>
        </div>
